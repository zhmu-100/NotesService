...
class NotificationAction(
  ...
  private val baseUrl = if (dbMode == "gateway") {
  ...
  override suspend fun createNotification(notification: Notification): Notification =
    withContext(Dispatchers.IO) {
      val requestBody = DbCreateRequest(
        table = "notifications",
        data = mapOf(
  ...
  override suspend fun getNotification(id: String): Notification? = withContext(Dispatchers.IO) {
    val requestBody = DbReadRequest(
      table = "notifications",
      columns = listOf("*"),
  ...
  override suspend fun listNotifications(userId: String, page: Int, pageSize: Int): List<Notification> =
    withContext(Dispatchers.IO) {
      val filtersMap = if (userId.isBlank()) null else mapOf("user_id" to userId)
      val requestBody = DbReadRequest(
        table = "notifications",
        columns = listOf("id", "user_id", "title", "description", "create_date", "notification_date"),
        filters = filtersMap
      )
      val url = "$baseUrl/read"
      val response: HttpResponse = httpClient.post(url) {
  ...
  override suspend fun performNotificationAction(
    ...
    val updated = when (action) {
      NotificationActionEnum.COMPLETE ->
        existing.copy(title = "[DONE] ${existing.title}")
      ...
      NotificationActionEnum.NOTIFICATION_ACTION_UNSPECIFIED ->
        existing
    }

    val requestBody = DbUpdateRequest(
      table = "notifications",
      data = mapOf(
        "user_id" to updated.userId,
  ...
  private fun LocalDateTime.plusSnooze(snooze: NotificationSnoozeEnum?): LocalDateTime {
    ...
    val duration: Duration = when (snooze) {
