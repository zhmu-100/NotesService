  ...
  routing {
    route("/notebook/notes") {
        ...
        try {
          ...
          logger.logActivity(
            ...
            additionalData = mapOf(
              "noteId" to createdNote.id,
              "title" to createdNote.title
            )
          )
          ...
          call.respondText("Error creating note: ${e.message}", status = HttpStatusCode.InternalServerError)
        }
      }
      ...
      get("{id}") {
        ...
        if (id == null) {
          logger.logActivity(
            event = "Invalid note ID in request",
            level = LogLevel.WARN
          )
      ...
      put("{id}") {
        ...
        if (id == null) {
          logger.logActivity(
            event = "Invalid note ID in update request",
            level = LogLevel.WARN
          )
          return@put call.respondText("Id is missing or invalid", status = HttpStatusCode.BadRequest)
        }
        ...
        try {
          ...
          call.respondText("Error updating note: ${e.message}", status = HttpStatusCode.InternalServerError)
        }
      }
      ...
      delete("{id}") {
        ...
        if (id == null) {
          logger.logActivity(
            event = "Invalid note ID in delete request",
            level = LogLevel.WARN
          )
          return@delete call.respondText("Id is missing or invalid", status = HttpStatusCode.BadRequest)
        }
        ...
        try {
          ...
          call.respondText("Error deleting note: ${e.message}", status = HttpStatusCode.InternalServerError)
        }
      }
    }
  }
