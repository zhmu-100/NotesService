    ...
    embeddedServer(Netty, port = port) {
          ...
          intercept(ApplicationCallPipeline.Monitoring) {
            ...
            try {
              ...
              logger.logActivity(
                event = "Запрос завершен: ${call.request.httpMethod.value} ${call.request.path()}",
                additionalData = mapOf(
                  "requestId" to requestId,
              ...
              if (count == 1 || count % 10 == 0) {
                ...
                // Логируем ошибку
                logger.logError(
                  event = "Ошибка при обработке запроса: ${call.request.httpMethod.value} ${call.request.path()} (повторений: $count)",
          ...
          environment.monitor.subscribe(ApplicationStopping) {
            logger.logActivity(
              event = "Приложение останавливается",
              level = LogLevel.WARN
            )
          }
          ...
          environment.monitor.subscribe(ApplicationStopPreparing) {
            ...
            logger.logActivity(
              event = "Application preparing to stop",
              level = LogLevel.WARN
            )
          }
...
/**
 * Создает экземпляр ILogger с обработкой ошибок для предотвращения сбоев приложения
